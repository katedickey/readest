...
export const listShares = async (cursor?: string | null): Promise<ShareListResponse> => {
  // The share API base is relative in dev (`/api/share`) and absolute in
  // prod, so we can't use `new URL()` here unconditionally — relative paths
  // throw "Invalid URL" without a base. Build the query string manually.
